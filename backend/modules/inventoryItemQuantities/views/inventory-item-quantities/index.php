...
<?= $this->render('@app/views/layouts/_inventory-tabs', ['activeTab' => 'quantities']) ?>
...
<div class="inv-page">
    <section class="fin-actions" aria-label="إجراءات" style="margin-bottom:16px">
        <?php if (Yii::$app->user->can(Permissions::INVENTORY_ITEMS_QUANTITY)): ?>
        <div class="fin-act-group">
            <?= Html::a('<i class="fa fa-plus"></i> <span>إضافة كمية</span>', ['create'], [
                'class' => 'fin-btn fin-btn--add', 'title' => 'إضافة كمية جديدة', 'role' => 'modal-remote',
            ]) ?>
        </div>
        <?php endif ?>
        <div class="fin-act-group">
    ...
    <div id="ajaxCrudDatatable">
        <?= GridView::widget([
            ...
            'summary' => '<span style="font-size:13px;color:#64748b"><i class="fa fa-table"></i> عرض <b>{begin}-{end}</b> من <b>{totalCount}</b> سجل</span>',
            'toolbar' => [
                ['content' =>
                    '{toggleData}' .
            ...
            'panel' => [
                'type' => 'default',
                'heading' => '<i class="fa fa-sort-numeric-asc"></i> كميات الأصناف <span class="badge" style="background:#0369a1;margin-right:6px">' . $searchCounter . '</span>',
            ],
            'pjax' => true,
